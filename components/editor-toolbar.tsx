...
import { FONT_OPTIONS, FONT_SIZES, type FontId, type FontSizeKey } from '@/lib/note-themes'
  ...
  return (
    <div className="w-full flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {/* Alignment */}
        <div className="flex items-center rounded-lg border border-border overflow-hidden">
          <ToolBtn active={align === 'left'}   onClick={() => onAlignChange('left')}   title="Izquierda" label={<AlignLeft  className="w-3.5 h-3.5" />} />
          <ToolBtn active={align === 'center'} onClick={() => onAlignChange('center')} title="Centro"    label={<AlignCenter className="w-3.5 h-3.5" />} />
          <ToolBtn active={align === 'right'}  onClick={() => onAlignChange('right')}  title="Derecha"   label={<AlignRight  className="w-3.5 h-3.5" />} />
        </div>
        ...
        {/* Undo / Redo */}
        <div className="flex items-center rounded-lg border border-border overflow-hidden">
          <ToolBtn active={false} onClick={onUndo} disabled={!canUndo} title="Deshacer (Ctrl+Z)" label={<Undo2 className="w-3.5 h-3.5" />} />
          <ToolBtn active={false} onClick={onRedo} disabled={!canRedo} title="Rehacer (Ctrl+Y)"  label={<Redo2 className="w-3.5 h-3.5" />} />
        </div>
        ...
        {/* Copy & Clear */}
        <ToolBtn active={false} onClick={onCopy}  title="Copiar texto" label={<Copy  className="w-3.5 h-3.5" />} standalone />
        <ToolBtn active={false} onClick={onClear} title="Borrar todo"  label={<Trash2 className="w-3.5 h-3.5" />} standalone />
      </div>

      {/* Row 2: tilt + char counter + download */}
      <div className="flex flex-wrap items-center gap-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <input
            ...
            title="Inclinación"
          />
          <span className="text-xs text-muted-foreground w-8">{tilt > 0 ? `+${tilt}` : tilt}°</span>
        </div>
          ...
          <div className="w-24 h-1 rounded-full bg-muted overflow-hidden">
            <div
              className={cn(
                ...
                charPct > 90 ? 'bg-destructive' : charPct > 70 ? 'bg-accent' : 'bg-foreground/40',
              )}
              style={{ width: `${Math.min(charPct, 100)}%` }}
            />
          </div>
          <span className={cn('text-xs tabular-nums', charPct > 90 ? 'text-destructive' : 'text-muted-foreground')}>
            {charCount}/{maxChars}
          </span>
        </div>
